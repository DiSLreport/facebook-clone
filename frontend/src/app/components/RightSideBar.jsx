...
const RightSideBar = () => {
    ...
    return (
        <motion.aside
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="flex text-lg front-semibold items-center">
                        <TrendingUp className="mr-2 h-5 w-5 text-priamry" />
                        Popular Sponsers
                    </CardTitle>
                </CardHeader>
                <CardContent>
                    <ul className="space-y-4">
                        {displaySponsers.map((sponsor, index) => (
                            <motion.li
                                key={sponsor.name}
                                initial={{ opacity: 0, y: 20 }}
                                animate={{ opacity: 1, y: 0 }}
                                transition={{ delay: index * 0.1 }}
                                className="flex flex-col items-center space-x-4 p-2 rponded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors duration-200">
                                <img src={sponsor.image}
                                    alt={sponsor.name}
                                    className="w-50 h-40 object-contain rounded md" />
                                <div className="flex-1">
                                    <h3 className="text-md font-semibold">{sponsor.name}</h3>
                                    <p className="text-sm text-gray-600 dark:text-400">{sponsor.description}</p>
                                    < a href={sponsor.website} target="_blank" className="text-primary text-sm flex items-center mt-1 hover:underline">
